...
void main() {
  ...
  while (step < maxStep && visibility > 0.0 / 255.0) {
    ...
    float densityTemp;
    densityTemp = texture(planeRgbDensity_1, planeUv[0]).a;
    densityTemp = denormalize(densityTemp, rangeDensityMin, rangeDensityMax);
    density += densityTemp;

    densityTemp = texture(planeRgbDensity_2, planeUv[1]).a;
    densityTemp = denormalize(densityTemp, rangeDensityMin, rangeDensityMax);
    density += densityTemp;

    densityTemp = texture(planeRgbDensity_3, planeUv[2]).a;
    densityTemp = denormalize(densityTemp, rangeDensityMin, rangeDensityMax);
    density += densityTemp;
#endif
...
#ifdef USE_TRIPLANE
      vec3 rgbTemp;
      rgbTemp = texture(planeRgbDensity_1, planeUv[0]).rgb;
      rgbTemp = denormalize(rgbTemp.rgb, rangeFeaturesMin, rangeFeaturesMax);
      rgb += rgbTemp;

      rgbTemp = texture(planeRgbDensity_2, planeUv[1]).rgb;
      rgbTemp = denormalize(rgbTemp.rgb, rangeFeaturesMin, rangeFeaturesMax);
      rgb += rgbTemp;

      rgbTemp = texture(planeRgbDensity_3, planeUv[2]).rgb;
      rgbTemp = denormalize(rgbTemp.rgb, rangeFeaturesMin, rangeFeaturesMax);
      rgb += rgbTemp;
#endif
