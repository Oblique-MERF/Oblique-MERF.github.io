...
struct QuadrantSetupResults_new{
    vec3 o;
    vec3 d;
    vec2 quadrantTMinMax;
}
...
QuadrantSetupResults quadrantSetup(vec3 o, vec3 d, float tP, float tQ) {
  ...
  //r.dContracted = normalize(zQ - r.o);
  r.d = d;
  ...
  r.quadrantTMinMaxContracted =
      rayAabbIntersection(aabbMin, aabbMax, r.oContracted, 1.0 / r.d);
  return r;
}
