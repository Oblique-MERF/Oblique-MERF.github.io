...
uniform float zinit;
uniform float occvoxelsize;

uniform highp sampler2DArray occupancyGrid_L0;
// uniform highp sampler2DArray occupancyGrid_L1;
// uniform highp sampler2DArray occupancyGrid_L2;
// uniform highp sampler2DArray occupancyGrid_L3;
uniform highp sampler2DArray occupancyGrid_L4;
...
uniform vec2 gridSizeOccupancy_L0;
uniform vec2 gridSizeOccupancy_L1;
uniform vec2 gridSizeOccupancy_L2;
uniform vec2 gridSizeOccupancy_L3;
uniform vec2 gridSizeOccupancy_L4;
...
uniform float triplaneGridSizeZ;
uniform float triplaneResolutionZinit;
// need to use texture arrays, otherwise we exceed max texture unit limit
// uniform highp sampler2DArray planeRgb_1;
// uniform highp sampler2DArray planeDensity_1;
uniform highp sampler2DArray planeFeatures_1;
// uniform highp sampler2DArray planeRgb_2;
// uniform highp sampler2DArray planeDensity_2;
uniform highp sampler2DArray planeFeatures_2;
// uniform highp sampler2DArray planeRgb_3;
// uniform highp sampler2DArray planeDensity_3;
uniform highp sampler2DArray planeFeatures_3;

uniform highp sampler2DArray planeRgbDensity_1;
uniform highp sampler2DArray planeRgbDensity_2;
uniform highp sampler2DArray planeRgbDensity_3;
#endif
`;
...
function createEmptyTriplaneTextureArray(width, height, format) {
  let texture = new THREE.DataTexture2DArray(null, width, height, 1);
 ...
 * @return {!THREE.DataTexture2DArray} Texture array of size three
 */
function createEmptyPlaneTextureArray(width, height, format, filter) {
  let texture = new THREE.DataTexture2DArray(null, width, height, 2);
  texture.format = format;
  texture.generateMipmaps = false;
  ...
  filenameToLinkPromise.then(filenameToLink => {
    ...
    for (let occupancyGridIndex = 0;
      ...
      let occupancyGridUrl = filenameToLinkTranslator.translate(
          'occupancy_plane_' + occupancyGridBlockSizes[occupancyGridIndex] +
          '.png');
    ...
    textPromises.then(parsed => {
      ...
      // Load triplanes.
      // let planeRgbTextures = [];
      // let planeDensityTextures = [];
      // let planeFeaturesTextures = [];

      // Load triplanes instead rgbtextures and densitytextures with rgbdensitytextures

      let planeRgbDensityTextures = [];
      let planeFeaturesTextures = [];

      let triplaneGridSize;
      if (useTriplane) {
        let triplaneResolution = sceneParams['triplane_resolution'];
        let triplaneResolutionZ = sceneParams['triplane_resolution_z'];
        let triplaneNumRes = 
            [triplaneResolutionZ, triplaneResolution,
              triplaneResolution, triplaneResolution,
              triplaneResolution, triplaneResolutionZ];
        // let triplaneNumRes = 
        //     [triplaneResolution, triplaneResolutionZ,
        //       triplaneResolution, triplaneResolution,
        //       triplaneResolutionZ, triplaneResolution];
        ...
        for (let planeIndex = 0;planeIndex < 3;planeIndex++) {
          ...
          //   triplaneNumRes[2 * planeIndex], triplaneNumRes[2 * planeIndex + 1], THREE.RGBFormat);
          // let planeDensityTexture = createEmptyTriplaneTextureArray(
          //   triplaneNumRes[2 * planeIndex], triplaneNumRes[2 * planeIndex + 1], THREE.RedFormat);
          // let planeFeaturesTexture = createEmptyTriplaneTextureArray(
          //   triplaneNumRes[2 * planeIndex], triplaneNumRes[2 * planeIndex + 1], THREE.RGBAFormat);
          // planeRgbTextures.push(planeRgbTexture)
          // planeDensityTextures.push(planeDensityTexture)
          // planeFeaturesTextures.push(planeFeaturesTexture)

          let planeRgbDensityTexture = createEmptyTriplaneTextureArray(
            triplaneNumRes[2 * planeIndex], triplaneNumRes[2 * planeIndex + 1], THREE.RGBAFormat);

          let planeFeaturesTexture = createEmptyTriplaneTextureArray(
            triplaneNumRes[2 * planeIndex], triplaneNumRes[2 * planeIndex + 1], THREE.RGBAFormat);
          planeRgbDensityTextures.push(planeRgbDensityTexture)
          planeFeaturesTextures.push(planeFeaturesTexture)

          let triplaneNumTexels = triplaneNumRes[2 * planeIndex] * triplaneNumRes[2 * planeIndex + 1];
          // planePromises.then(planeData => {
          //   let planeRgbAndDensity = planeData[2 * planeIndex]
          //   let planeRgbStack = new Uint8Array(triplaneNumTexels * 3);
          //   let planeDensityStack = new Uint8Array(triplaneNumTexels);
          //   for (let j = 0; j < triplaneNumTexels; j++){
          //     planeRgbStack[j * 3 + 0] = planeRgbAndDensity[j * 4 + 0];
          //     planeRgbStack[j * 3 + 1] = planeRgbAndDensity[j * 4 + 1];
          //     planeRgbStack[j * 3 + 2] = planeRgbAndDensity[j * 4 + 2];
          //     planeDensityStack[j] = planeRgbAndDensity[j * 4 + 3];
          //   }
          ...
          // });
          planePromises.then(planeData => {
            let planeRgbDensity = planeData[2 * planeIndex]
            let planeRgbDensityStack = new Uint8Array(triplaneNumTexels * 4);
            for (let j = 0; j < triplaneNumTexels; j++){
              planeRgbDensityStack[j * 4 + 0] = planeRgbDensity[j * 4 + 0];
              planeRgbDensityStack[j * 4 + 1] = planeRgbDensity[j * 4 + 1];
              planeRgbDensityStack[j * 4 + 2] = planeRgbDensity[j * 4 + 2];
              planeRgbDensityStack[j * 4 + 3] = planeRgbDensity[j * 4 + 3];
            }
            planeRgbDensityTexture.image.data = planeRgbDensityStack;
            planeRgbDensityTexture.needsUpdate = true;
          });
          planePromises.then(planeData => {
            let planeFeatures = planeData[2 * planeIndex + 1]
            let planeFeaturesStack = new Uint8Array(triplaneNumTexels * 4);
            for (let j = 0; j < triplaneNumTexels; j++){
              planeFeaturesStack[j * 4 + 0] = planeFeatures[j * 4 + 0];
              planeFeaturesStack[j * 4 + 1] = planeFeatures[j * 4 + 1];
              planeFeaturesStack[j * 4 + 2] = planeFeatures[j * 4 + 2];
              planeFeaturesStack[j * 4 + 3] = planeFeatures[j * 4 + 3];
            }
      ...
      // }
      let resolutionToUse = sceneParams['occgridsize'];
      let voxelSizeToUse = sceneParams['occvoxelsize'];
      ...
      for (let occupancyGridIndex = 0;
        ...
        // scene contraction.
        let v = Math.ceil(8.0 * resolutionToUse / occupancyGridBlockSize);
        let occupancyGridTexture = createEmptyPlaneTextureArray(
          v, v, THREE.RedFormat, THREE.NearestFilter);
        occupancyGridTextures.push(occupancyGridTexture);
        occupancyGridSizes.push(new THREE.Vector2(v, v));
        occupancyVoxelSizes.push(voxelSizeToUse * occupancyGridBlockSize / 8.0);
        occupancyGridPromises[occupancyGridIndex].then(
            occupancyGridImageFourChannels => {
              onImageLoaded();
              let baseOffset = v * v;
              let occupancyGridImage = new Uint8Array(baseOffset * 2);
              for (let j = 0; j < v * v; j++) {
                occupancyGridImage[j] = occupancyGridImageFourChannels[4 * j];
                occupancyGridImage[j + baseOffset] = occupancyGridImageFourChannels[4 * j + 3];
              }
      ...
      let minPosition = new THREE.Vector3(-1.0, -1.0, -1.0);
      ...
      let rayMarchUniforms = {
        ...
        'zinit': {'value': sceneParams['z_min_init']},
        'occvoxelsize': {'value': sceneParams['occvoxelsize']},
      };

      if (useTriplane) {
        // let triplaneUniforms = {
        //   'planeRgb_1': {'value': planeRgbTextures[0]},
        //   'planeDensity_1': {'value': planeDensityTextures[0]},
        //   'planeFeatures_1': {'value': planeFeaturesTextures[0]},
        //   'planeRgb_2': {'value': planeRgbTextures[1]},
        //   'planeDensity_2': {'value': planeDensityTextures[1]},
        //   'planeFeatures_2': {'value': planeFeaturesTextures[1]},
        //   'planeRgb_3': {'value': planeRgbTextures[2]},
        //   'planeDensity_3': {'value': planeDensityTextures[2]},
        //   'planeFeatures_3': {'value': planeFeaturesTextures[2]},
        //   'triplaneGridSize': {'value': triplaneGridSize},
        //   'triplaneVoxelSize': {'value': sceneParams['triplane_voxel_size']},
        //   'triplaneGridSizeZ': {'value': sceneParams['triplane_resolution_z']},
        //   'triplaneResolutionZinit': {'value': sceneParams['triplane_resolution_z_init']},
        // };

        let triplaneUniforms = {
          'planeRgbDensity_1': {'value': planeRgbDensityTextures[0]},
          'planeFeatures_1': {'value': planeFeaturesTextures[0]},
          'planeRgbDensity_2': {'value': planeRgbDensityTextures[1]},
          'planeFeatures_2': {'value': planeFeaturesTextures[1]},
          'planeRgbDensity_3': {'value': planeRgbDensityTextures[2]},
          'planeFeatures_3': {'value': planeFeaturesTextures[2]},
          'triplaneGridSize': {'value': triplaneGridSize},
          'triplaneVoxelSize': {'value': sceneParams['triplane_voxel_size']},
          'triplaneGridSizeZ': {'value': sceneParams['triplane_resolution_z']},
...
function initFromParameters() {
  ...
  const nearPlane = parseFloat(params.get('near') || 0.1);
